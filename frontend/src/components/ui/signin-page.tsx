...
export const Button = ({ 
  ...
  const variants = {
    primary: "bg-primary text-primary-foreground shadow-lg hover:opacity-90 disabled:opacity-50",
    secondary: "bg-secondary text-secondary-foreground hover:bg-secondary/80 disabled:opacity-50",
    outline: "border border-border bg-background hover:bg-secondary text-foreground disabled:opacity-50"
  };
...
export const SocialButton = ({ provider, onClick, children }: SocialButtonProps) => {
  const icons = {
...
export const ProgressDots = ({ count = 3, activeIndex = 2, color = "white" }: ProgressDotsProps) => (
...
export const HeroSection = ({ title, description, icon, showProgress = true }: HeroSectionProps) => (
...
export interface GradientBackgroundProps {
  children: React.ReactNode;
  variant?: "default" | "dark" | "light" | "lilac";
}

export const GradientBackground = ({ children, variant = "dark" }: GradientBackgroundProps) => {
  const variants = {
    ...
    lilac: "bg-gradient-to-br from-purple-700 via-fuchsia-600 to-purple-900"
  };
...
export const FormFooter = ({ text, linkText, linkHref, onLinkClick }: FormFooterProps) => (
  <p className="mt-6 text-center text-sm text-muted-foreground">
    {text}{" "}
    {onLinkClick ? (
      <button type="button" onClick={onLinkClick} className="text-primary hover:opacity-80 font-medium transition-opacity duration-300">
        {linkText}
      </button>
    ) : (
      <a href={linkHref} className="text-primary hover:opacity-80 font-medium transition-opacity duration-300">
        {linkText}
      </a>
    )}
  </p>
...
const SignIn = ({ onNavigate }: { onNavigate?: (view: string) => void }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row w-full">
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <Card className="p-6 sm:p-8 shadow-sm">
            ...
            <FormFooter 
              ...
              linkText="Sign up"
              onLinkClick={() => onNavigate && onNavigate("signup")}
              linkHref="/signup"
            />
          </Card>
        </div>
      ...
      <GradientBackground variant="lilac">
        <HeroSection
          title="Welcome to Klyro"
          description="Your central hub for Enterprise Brand Intelligence. Securely manage your brand's future."
          icon={
